...
<div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Live Tv Page</h4>
                    <a href="{{route('Setting.LiveTv.create')}}" class="btn btn-primary" style="float: right;">Add Live</a>
                    <div class="table-responsive">
                      <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach($livetvs as $row)
                                <tr>
                                    <td>#</td>
                                    <td>{{$row->embed_code}}</td>
                                    <td>{{$row->status}}</td>
                                    <td>
                                        <a href="{{route('Setting.LiveTv.delete',$row->id)}}" class="btn btn-danger">Delete</a>
                                        <a href="{{route('Setting.Livetv.edit',$row->id)}}" class="btn btn-primary">Edit</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody> 
